...
public class SearchDrugstore {

	Map<String, DrugstorePage> pagelist = new HashMap<String, DrugstorePage>();
	List<DrugstorePage> maillist = new ArrayList<DrugstorePage>();

	public static void main(String[] args) throws Exception {
		ProxyUtli.setProxy(true);
		SearchDrugstore af = new SearchDrugstore();
	...
	public void searchsite() {
		System.out.println("Checking Drugstore pages...");
		syncDb();
		check();
		removeDeletePage();
	}
	...
	private void syncDb() {
		...
		List<DrugstorePage> urllist = jsp.loadAll();
		if (urllist != null & urllist.size() != 0)
			maillist = urllist;
	}

	private void sendmail(DrugstorePage oldpage, DrugstorePage page) {

		Mail m = new Mail();
		if (!oldpage.instock && page.instock) {
			m.mail_subject = "Drugstore价格提醒,您的产品<" + page.prodname + ">,到货！";
		} else {
			m.mail_subject = "Drugstore价格提醒,您的产品<" + page.prodname + ">,降价！";
		}
	...
	private void check() {
		if (maillist != null)
			System.out.println("Checking Drugstore pages size:"
					+ maillist.size());
		for (int i = 0; i < maillist.size(); i++) {
			DrugstorePage page = maillist.get(i);
			try {

				ParserDrugstorePage parserDrugstorePage = new ParserDrugstorePage(
						page);
				...
				DrugstorePage oldpage = pagelist.get(key);
				if (oldpage != null) {
				...
				SystemMail.sendSystemMail("Error when get Drugstore page."
						+ page.url + ";" + e.toString());
			}
		}
	}

	private void checkPageChange(DrugstorePage oldpage, DrugstorePage page) {
	...
	private void removeDeletePage() {
		...
		while (entryKeyIterator.hasNext()) {
			...
			DrugstorePage page = pagelist.get(key);
			if (!existinDb(page)) {
	...
	private boolean existinDb(DrugstorePage page) {

		for (int i = 0; i < maillist.size(); i++) {
			DrugstorePage pageindb = maillist.get(i);
