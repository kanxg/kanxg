...
public class SearchBeauty {

	Map<String, BeautyPage> pagelist = new HashMap<String, BeautyPage>();
	List<BeautyPage> maillist = new ArrayList<BeautyPage>();

	public static void main(String[] args) throws Exception {
		ProxyUtli.setProxy(true);
		SearchBeauty af = new SearchBeauty();
	...
	public void searchsite() {
		System.out.println("Checking Beauty pages...");
		syncDb();
		check();
		removeDeletePage();
	}
	...
	private void syncDb() {
		...
		List<BeautyPage> urllist = jsp.loadAll();
		if (urllist != null & urllist.size() != 0)
			maillist = urllist;
	}

	private void sendmail(BeautyPage oldpage, BeautyPage page) {

		Mail m = new Mail();
		if (!oldpage.instock && page.instock) {
			m.mail_subject = "Beauty价格提醒,您的产品<" + page.prodname + ">,到货！";
		} else {
			m.mail_subject = "Beauty价格提醒,您的产品<" + page.prodname + ">,降价！";
		}
	...
	private void check() {
		if (maillist != null)
			System.out.println("Checking Beauty pages size:"
					+ maillist.size());
		for (int i = 0; i < maillist.size(); i++) {
			BeautyPage page = maillist.get(i);
			try {

				ParserBeautyPage parserBeautyPage = new ParserBeautyPage(
						page);
				...
				BeautyPage oldpage = pagelist.get(key);
				if (oldpage != null) {
				...
				SystemMail.sendSystemMail("Error when get Beauty page."
						+ page.url + ";" + e.toString());
			}
		}
	}

	private void checkPageChange(BeautyPage oldpage, BeautyPage page) {
	...
	private void removeDeletePage() {
		...
		while (entryKeyIterator.hasNext()) {
			...
			BeautyPage page = pagelist.get(key);
			if (!existinDb(page)) {
	...
	private boolean existinDb(BeautyPage page) {

		for (int i = 0; i < maillist.size(); i++) {
			BeautyPage pageindb = maillist.get(i);
