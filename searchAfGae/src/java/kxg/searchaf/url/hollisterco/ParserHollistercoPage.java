...
public class ParserHollistercoPage {
	public HollistercoPage page;
	public HashMap<Integer, HollistercoProduct> allprolist;
	public HollistercoProduct product;

	public ParserHollistercoPage(HollistercoPage page,
			HashMap<Integer, HollistercoProduct> allprolist) {
		this.page = page;
	...
	public void checkprice() throws Exception {
		...
		String titleText = "";
		// System.out.println("checking hollisterco url [" + page.url + "]");

		// HttpClient httpclient = new HttpClient();
		//
		// if (HollistercoConstant.usingProxy) {
		...
		// + redirectURL);
		// }
		if (redirectURL.indexOf(HollistercoConstant.checkingURLhost) < 0) {
			// System.out.println("URL:" + page.url
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tag instanceof TitleTag) {
				...
				titleText = title.toPlainTextString()
						.replaceAll("HollisterCo.com", "").trim();
				...
				if ("product".equals(div_class)
					...
					// found one product
					if (HollistercoConstant.isIgnoreProduct(currentCategory)) {
						// ignore these products under Featured Items.
						continue;
					} else {
						product = new HollistercoProduct();
	...
	private String processHTML(Node node) {
		...
		// "http://www.hollisterco.com/webapp/wcs/stores/servlet/ProductDisplay");
		html = html
	...
	public static void main(String[] args) throws Exception {
		...
		// String url =
		// "http://www.hollisterco.com/webapp/wcs/stores/servlet/CategoryDisplay?catalogId=10901&storeId=10051&langId=-1&topCategoryId=12202&categoryId=12204&parentCategoryId=12204";
		...
		HashMap<Integer, HollistercoProduct> allmenoldprolist = new HashMap<Integer, HollistercoProduct>();
		ParserHollistercoPage parse = new ParserHollistercoPage(page,
				allmenoldprolist);
		parse.checkprice();
		...
		for (int i = 0; i < matchprolist.size(); i++) {
			HollistercoProduct afProduct = matchprolist.get(i);
