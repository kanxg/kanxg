...
public class SearchSaks {
	...
	Map<String, SaksPage> pagelist = new HashMap<String, SaksPage>();
	List<SaksPage> maillist = new ArrayList<SaksPage>();

	public static void main(String[] args) throws Exception {
		ProxyUtli.setProxy(true);
		SearchSaks af = new SearchSaks();
	...
	public void searchsite() {
		System.out.println("Checking Saks pages...");
		syncDb();
		check();
		removeDeletePage();
	}
	...
	private void syncDb() {
		...
		List<SaksPage> urllist = jsp.loadAll();
		if (urllist != null & urllist.size() != 0)
			maillist = urllist;
	}

	private void sendmail(SaksPage oldpage, SaksPage page) {

		Mail m = new Mail();
		if (!oldpage.instock && page.instock) {
			m.mail_subject = "Saks价格提醒,您的产品<" + page.prodname + ">,到货！";
		} else {
			m.mail_subject = "Saks价格提醒,您的产品<" + page.prodname + ">,降价！";
		}
	...
	private void check() {
		if (maillist != null)
			System.out.println("Checking Saks pages size:"
					+ maillist.size());
		for (int i = 0; i < maillist.size(); i++) {
			SaksPage page = maillist.get(i);
			try {

				ParserSaksPage parserSaksPage = new ParserSaksPage(
						page);
				...
				SaksPage oldpage = pagelist.get(key);
				if (oldpage != null) {
				...
				SystemMail.sendSystemMail("Error when get Saks page."
						+ page.url + ";" + e.toString());
			}
		}
	}

	private void checkPageChange(SaksPage oldpage, SaksPage page) {
	...
	private void removeDeletePage() {
		...
		while (entryKeyIterator.hasNext()) {
			...
			SaksPage page = pagelist.get(key);
			if (!existinDb(page)) {
	...
	private boolean existinDb(SaksPage page) {

		for (int i = 0; i < maillist.size(); i++) {
			SaksPage pageindb = maillist.get(i);
