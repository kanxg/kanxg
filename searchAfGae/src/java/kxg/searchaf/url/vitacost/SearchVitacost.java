...
public class SearchVitacost {

	Map<String, VitacostPage> pagelist = new HashMap<String, VitacostPage>();
	List<VitacostPage> maillist = new ArrayList<VitacostPage>();

	public static void main(String[] args) throws Exception {
		ProxyUtli.setProxy(true);
		SearchVitacost af = new SearchVitacost();
	...
	public void searchsite() {
		System.out.println("Checking Vitacost pages...");
		syncDb();
		check();
		removeDeletePage();
	}
	...
	private void syncDb() {
		...
		List<VitacostPage> urllist = jsp.loadAll();
		if (urllist != null & urllist.size() != 0)
			maillist = urllist;
	}

	private void sendmail(VitacostPage oldpage, VitacostPage page) {

		Mail m = new Mail();
		if (!oldpage.instock && page.instock) {
			m.mail_subject = "Vitacost价格提醒,您的产品<" + page.prodname + ">,到货！";
		} else {
			m.mail_subject = "Vitacost价格提醒,您的产品<" + page.prodname + ">,降价！";
		}
	...
	private void check() {
		if (maillist != null)
			System.out.println("Checking Vitacost pages size:"
					+ maillist.size());
		for (int i = 0; i < maillist.size(); i++) {
			VitacostPage page = maillist.get(i);
			try {
				...
				VitacostPage oldpage = pagelist.get(key);
				if (oldpage != null) {
				...
				SystemMail.sendSystemMail("Error when get Vitacost page."
						+ page.url + ";" + e.toString());
			}
		}
	}

	private void checkPageChange(VitacostPage oldpage, VitacostPage page) {
	...
	private void removeDeletePage() {
		...
		while (entryKeyIterator.hasNext()) {
			...
			VitacostPage page = pagelist.get(key);
			if (!existinDb(page)) {
	...
	private boolean existinDb(VitacostPage page) {

		for (int i = 0; i < maillist.size(); i++) {
			VitacostPage pageindb = maillist.get(i);
