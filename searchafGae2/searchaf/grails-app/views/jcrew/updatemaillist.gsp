...
<%@ page import="kxg.searchaf.url.jcrew.JcrewMailListJSP" %>
<%@ page import="kxg.searchaf.url.jcrew.JcrewMailList" %>
...
<html lang="en">
 ...
 <form id="form1" class="form-horizontal" action="<%=request.getContextPath()%>/jcrew/updatemaillist" method="post">
...
$( document ).ready(function(){
  ...
   $("#loadbutton").click(function() {
  	  $("#form1").attr("action", "<%=request.getContextPath()%>/jcrew/updatemaillist?action=load");
   });
  ...
   $("#updatebutton").click(function() {
	...
	if(Number(tem4)<0.01 || Number(tem2)>1){
		...
		return false;
   		...
   		$("#form1").attr("action", "<%=request.getContextPath()%>/jcrew/updatemaillist?action=update");
   });
