...
<%@ page import="kxg.searchaf.url.ralphlauren.SearchRalphlaurenJSP" %>
<%@ page import="kxg.searchaf.url.ralphlauren.RalphlaurenProduct" %>
<%@ page import="kxg.searchaf.url.ralphlauren.RalphlaurenConstant" %>
...
<%
    ...
    if(showdiscount==null){
		showdiscount=String.valueOf(RalphlaurenConstant.warningdiscount);
		firstload=true;
	}
	...
	List<RalphlaurenProduct> matchprolist = null;
	if(!firstload ){
...
%>
    ...
    <form name="frm" action="<%=request.getContextPath()%>/rl/searchboy" method="post" onsubmit="return checkinput()">
      <table>
      <tr>
  ...
  <tr>
     ...
     if(matchprolist!=null){
      for (int i = 0; i < matchprolist.size(); i++) {
    	  RalphlaurenProduct product= matchprolist.get(i);
    	  int mod=i % 4 ;
     	  if(mod==0){
       %>
       	 <tr width='100%'>  
