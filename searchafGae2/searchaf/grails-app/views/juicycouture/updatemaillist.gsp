...
<html lang="en">
 ...
 <form id="form1" class="form-horizontal" action="<%=request.getContextPath()%>/juicycouture/updatemaillist" method="post">
...
$( document ).ready(function(){
  ...
   $("#loadbutton").click(function() {
  	  $("#form1").attr("action", "<%=request.getContextPath()%>/juicycouture/updatemaillist?action=load");
   });
  ...
   $("#updatebutton").click(function() {
	...
	if(Number(tem2)<0.01 || Number(tem2)>1){
		...
		return false;
   		...
   		$("#form1").attr("action", "<%=request.getContextPath()%>/juicycouture/updatemaillist?action=update");
   });
