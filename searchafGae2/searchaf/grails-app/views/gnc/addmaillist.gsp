...
<%@ page import="kxg.searchaf.url.gnc.GncPageJSP" %>
<%@ page import="kxg.searchaf.url.gnc.GncPage" %>
<%@ page import="java.util.*" %>
 <%request.setCharacterEncoding("utf-8");%>  
 
<%	
	 String ErrMsg="";
     List<GncPage> pagelist=null;
	 ...
	 if(mailaddress!=null){
	 	...
	 	if("add".equals(action)){
	 		 ...
	 		 }else if("notgnc".equals(ErrMsg)){
	 		 	ErrMsg="当前只支持gnc.com,请输入正确的链接!";
...
<html lang="en">
 ...
 <form id="form1" class="form-horizontal" action="<%=request.getContextPath()%>/gnc/addmaillist" method="post">
...
$( document ).ready(function(){
  ...
   $("#addbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/gnc/addmaillist?action=add");
   ...
   $("#loadbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/gnc/addmaillist?action=load");
   ...
   $("#deletebutton<%=i%>").click(function() {
   		...
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/gnc/addmaillist?action=delete");
   		$("#form1").submit();
   });
   
   $("#querybutton<%=i%>").click(function() {
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/gnc/testurl");
   		$("#form1").submit();
    });
