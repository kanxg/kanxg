...
<%@ page import="kxg.searchaf.url.sixpm.SixpmPageJSP" %>
<%@ page import="kxg.searchaf.url.sixpm.SixpmPage" %>
<%@ page import="java.util.*" %>
<%request.setCharacterEncoding("utf-8");%>  
 
<%	
	 String ErrMsg="";
     List<SixpmPage> pagelist=null;
	 ...
	 if(mailaddress!=null){
	 	...
	 	if("add".equals(action)){
	 		 ...
	 		 }else if("notsixpm".equals(ErrMsg)){
	 		 	ErrMsg="当前只支持6pm.com,请输入正确的链接!";
...
<html lang="en">
 ...
 <form id="form1" class="form-horizontal" action="<%=request.getContextPath()%>/sixpm/addmaillist" method="post">
...
$( document ).ready(function(){
  ...
   $("#addbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/sixpm/addmaillist?action=add");
   ...
   $("#loadbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/sixpm/addmaillist?action=load");
   ...
   $("#deletebutton<%=i%>").click(function() {
   		...
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/sixpm/addmaillist?action=delete");
   		$("#form1").submit();
   });
   
   $("#querybutton<%=i%>").click(function() {
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/sixpm/testurl");
   		$("#form1").submit();
    });
