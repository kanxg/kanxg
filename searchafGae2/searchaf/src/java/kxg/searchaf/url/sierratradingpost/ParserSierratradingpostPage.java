...
import kxg.searchaf.url.sierratradingpost.ParserSierratradingpostPage;
import kxg.searchaf.url.sierratradingpost.SierratradingpostPage;
...
public class ParserSierratradingpostPage {
	public SierratradingpostPage page;

	public ParserSierratradingpostPage(SierratradingpostPage page) {
		this.page = page;
	};

	public void checkprice() throws Exception {
		// System.out.println("checking sierratradingpost url:" +
		// page.getUrl());

		String cookies = ""; // SierratradingpostLogin.getCookies();
		...
		NodeFilter name_filter3 = new AndFilter(
				new NodeClassFilter(Span.class), new HasAttributeFilter("id",
						"displayPrice"));
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tag instanceof TitleTag) {
				...
				page.title = d.getTitle()
						.replaceAll("| sierratradingpost.com", "").trim();
			} else if (tag instanceof Span) {
				Span d = (Span) tag;
				String sStr = d.getStringText();
				// System.out.println(sStr);
				if ("displayPrice".equalsIgnoreCase(d.getAttribute("id"))) {
	...
	private float getPrice(String priceStr) throws Exception {
		...
		return getprice(priceStr.replaceAll("SALE:", "").trim());
	}
	...
	public static void main(String[] args) throws Exception {
		...
		sierratradingpostPage.url = "http://www.sierratradingpost.com/jacob-ash-waterproof-ski-mittens-insulated-for-kids~p~55197/?filterString=Or(kids-hats-and-beanies~d~5700%2Ckids-gloves-and-mittens~d~5699)%2Fclothing~d~5%2F&colorFamily=74";
		ParserSierratradingpostPage parserSierratradingpostPage = new ParserSierratradingpostPage(
				sierratradingpostPage);
		parserSierratradingpostPage.checkprice();
		System.out.println(sierratradingpostPage);
	}
}
