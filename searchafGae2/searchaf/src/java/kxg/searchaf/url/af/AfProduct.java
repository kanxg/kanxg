...
public class AfProduct implements Comparable<Object> {
	...
	public String toJspString() {
		...
		if (inventoryList != null && inventoryList.size() > 0) {
			...
			for (int i = 0; i < inventoryList.size(); i++) {
				...
				String additemurl = AfConstant.addtoCartUrl + "&productId="
						+ this.productid + "&partNumber=" + inventory.sku;
	...
	private static int getSequence(String name) {
		// men
		// category
		if ("TEES".equalsIgnoreCase(name))
			return 9;
		if ("GRAPHIC TEES".equalsIgnoreCase(name))
			return 10;
		if ("SHORT SLEEVE TEES".equalsIgnoreCase(name))
			return 20;
		if ("LONG SLEEVE TEES".equalsIgnoreCase(name))
			return 30;
