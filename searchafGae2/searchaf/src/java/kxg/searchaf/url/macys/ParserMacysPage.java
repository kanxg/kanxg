...
import kxg.searchaf.url.macys.ParserMacysPage;
import kxg.searchaf.url.macys.MacysPage;
...
public class ParserMacysPage {
	public MacysPage page;

	public ParserMacysPage(MacysPage page) {
		this.page = page;
	};

	public void checkprice() throws Exception {
		// System.out.println("checking macys url:" + page.getUrl());

		String cookies = ""; // MacysLogin.getCookies();
		...
		NodeFilter name_filter3 = new AndFilter(new NodeClassFilter(Div.class),
				new HasAttributeFilter("class", "standardProdPricingGroup"));
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tag instanceof TitleTag) {
				...
				page.title = d.getTitle().replaceAll("| macys.com", "").trim();
			} else if (tag instanceof Div) {
				Div d = (Div) tag;

				if ("standardProdPricingGroup".equalsIgnoreCase(d
						.getAttribute("class"))) {
	...
	public static void main(String[] args) throws Exception {
		...
		macysPage.url = "http://www1.macys.com/shop/product/clinique-scruffing-lotion-for-men-35?ID=531159&CategoryID=30078#fn=sp%3D1%26spc%3D24%26kws%3Dclinique%20men%26slotId%3D17";
		ParserMacysPage parserMacysPage = new ParserMacysPage(macysPage);
		parserMacysPage.checkprice();
		System.out.println(macysPage);
	}
}
