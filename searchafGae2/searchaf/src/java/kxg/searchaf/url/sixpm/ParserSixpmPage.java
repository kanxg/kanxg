...
import kxg.searchaf.url.sixpm.ParserSixpmPage;
import kxg.searchaf.url.sixpm.SixpmPage;
...
public class ParserSixpmPage {
	public SixpmPage page;

	public ParserSixpmPage(SixpmPage page) {
		this.page = page;
	};

	public void checkprice() throws Exception {
		// System.out.println("checking sixpm url:" + page.getUrl());

		String cookies = ""; // SixpmLogin.getCookies();
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tag instanceof TitleTag) {
				...
				page.title = d.getTitle().replaceAll("- 6pm.com", "").trim();
			} else if (tag instanceof Div) {
				Div d = (Div) tag;
				String sStr = d.getStringText();
				// System.out.println(sStr);
				if ("price".equalsIgnoreCase(d.getAttribute("class"))) {
	...
	public static void main(String[] args) throws Exception {
		...
		sixpmPage.url = "http://www.6pm.com/maidenform-custom-lift-174-tailored-t-shirt-bra-zebra-fun-house";
		ParserSixpmPage parserSixpmPage = new ParserSixpmPage(sixpmPage);
		parserSixpmPage.checkprice();
		System.out.println(sixpmPage);
	}
}
