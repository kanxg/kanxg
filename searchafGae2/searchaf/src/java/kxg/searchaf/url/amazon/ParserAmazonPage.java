...
public class ParserAmazonPage {
	...
	public ParserAmazonPage(AmazonPage page) {
		this.page = page;
	};
	...
	public void checkprice() throws Exception {
		...
		NodeFilter name_filter3 = new AndFilter(new NodeClassFilter(Div.class),
				new HasAttributeFilter("id", "price"));

		NodeFilter name_filter4 = new AndFilter(new NodeClassFilter(Div.class),
				new HasAttributeFilter("id", "availability_feature_div"));
		...
		lastFilter.setPredicates(new NodeFilter[] {
				...
				name_filter3, name_filter4, name_filter5 });
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tag instanceof TitleTag) {
				...
				if (p instanceof Div) {
					...
					if (sStr.indexOf("Ships from and sold by <b>Amazon.com") > 0) {
				...
				Div d = (Div) tag;
				if ("price".equalsIgnoreCase(d.getAttribute("id"))) {
					page.price = getPrice(d);
				} else if ("availability_feature_div".equalsIgnoreCase(d
						.getAttribute("id"))) {
					...
					if (sStr.indexOf("Ships from and sold by Amazon.com") > 0) {
						page.salerisAmazon = true;
					}
				}
	...
	private float getPrice(Node node) throws Exception {
		...
		for (int i = 0; i < productvalue.size(); i++) {
			...
			if (tmp.indexOf("Price") == 0) {
				price = productvalue.get(i + 1);
				break;
			}
		}
		if (price.indexOf("To see our price, add this item to your cart") >= 0) {
			return 0;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		amazonPage.url = "http://www.amazon.com/gp/product/B008SH86VC/ref=ox_sc_sfl_title_2?ie=UTF8&psc=1";
