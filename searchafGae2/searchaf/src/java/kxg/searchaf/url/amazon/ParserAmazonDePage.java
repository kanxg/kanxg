...
public class ParserAmazonDePage {
	public AmazonPage page;

	public ParserAmazonDePage(AmazonPage page) {
		this.page = page;
	};
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tag instanceof TitleTag) {
				...
				if (p instanceof Div) {
					...
					if (sStr.indexOf("Auf Lager") > 0
							|| sStr.indexOf("auf Lager") > 0) {
						page.instock = true;
					}
					if (sStr.indexOf("Verkauf und Versand durch <b>Amazon</b>") > 0) {
	...
	private float getprice(String priceStr) throws Exception {
		priceStr = priceStr.replaceAll("EUR", "").replaceAll(",", ".").trim();
		float returnvalue = 0;
		try {
			// listprice= $80
	...
	public static void main(String[] args) throws Exception {
		...
		amazonPage.url = "http://www.amazon.de/gp/product/B004X0XALE/ref=ox_sc_sfl_title_4?ie=UTF8&psc=1";
		ParserAmazonDePage parserAmazonPage = new ParserAmazonDePage(amazonPage);
