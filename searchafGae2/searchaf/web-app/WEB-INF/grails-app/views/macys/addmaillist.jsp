...
<%@ page import="kxg.searchaf.url.macys.MacysPageJSP" %>
<%@ page import="kxg.searchaf.url.macys.MacysPage" %>
<%@ page import="java.util.*" %>
 <%request.setCharacterEncoding("utf-8");%>  
 
<%	
	 String ErrMsg="";
     List<MacysPage> pagelist=null;
	 ...
	 if(mailaddress!=null){
	 	...
	 	if("add".equals(action)){
	 		 ...
	 		 }else if("notmacys".equals(ErrMsg)){
	 		 	ErrMsg="当前只支持macys.com,请输入正确的链接!";
...
<html lang="en">
 ...
 <form id="form1" class="form-horizontal" action="<%=request.getContextPath()%>/macys/addmaillist" method="post">
...
$( document ).ready(function(){
  ...
   $("#addbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/macys/addmaillist?action=add");
   ...
   $("#loadbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/macys/addmaillist?action=load");
   ...
   $("#deletebutton<%=i%>").click(function() {
   		...
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/macys/addmaillist?action=delete");
   		$("#form1").submit();
   });
   
   $("#querybutton<%=i%>").click(function() {
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/macys/testurl");
   		$("#form1").submit();
    });
