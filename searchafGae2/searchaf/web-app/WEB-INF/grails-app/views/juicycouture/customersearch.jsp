...
<%@ page import="kxg.searchaf.url.juicycouture.SearchJuicycoutureJSP" %>
<%@ page import="kxg.searchaf.url.juicycouture.JuicycoutureProduct" %>
<%@ page import="kxg.searchaf.url.juicycouture.JuicycoutureConstant" %>
...
%>
    ...
    <form name="frm" action="<%=request.getContextPath()%>/juicycouture/customersearch" method="post" onsubmit="return checkinput()">
      <table>
       <tr>
      		<td>搜索URL:<input type="text" name="searchurl" value="<%=searchurl%>" size="160"/></td>
      		<td>输入Juicycouture 网站任意链接</td>
      </tr>
      <tr>
...
function back(){
	document.frm.action="juicycouture.html";
	document.frm.submit();
}
