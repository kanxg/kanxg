...
<%	
	 String ErrMsg="";
     List<DrugstorePage> pagelist=null;
	 ...
	 if(mailaddress!=null){
	 	...
	 	if("add".equals(action)){
	 		 ...
	 		 }else if("notdrugstore".equals(ErrMsg)){
	 		 	ErrMsg="当前只支持drugstore.com,请输入正确的链接!";
...
<html lang="en">
 ...
 <form id="form1" class="form-horizontal" action="<%=request.getContextPath()%>/drugstore/addmaillist" method="post">
...
$( document ).ready(function(){
  ...
   $("#addbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/drugstore/addmaillist?action=add");
   ...
   $("#loadbutton").click(function() {
   		$("#form1").attr("action", "<%=request.getContextPath()%>/drugstore/addmaillist?action=load");
   ...
   $("#deletebutton<%=i%>").click(function() {
   		...
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/drugstore/addmaillist?action=delete");
   		$("#form1").submit();
   });
   
   $("#querybutton<%=i%>").click(function() {
   		$("#url").val("<%=apage.url%>");
   		$("#form1").attr("action", "<%=request.getContextPath()%>/drugstore/testurl");
   		$("#form1").submit();
    });
